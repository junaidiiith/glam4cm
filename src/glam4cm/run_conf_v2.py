                            ...
                            for gnn_model in gnn_models:
                                for j in range(len((gnn_updates))):
                                    ...
                                    if "use_embeddings" in gnn_updates[:j+1]:
                                        ...
                                        gnn_params_str += f'--use_embeddings --ckpt={get_embed_model_name(dataset, gnn_task_id, node_cls_label, edge_cls_label)} ' 
...
df = pd.DataFrame('logs/run_configs.csv') \
    if os.path.exists('logs/run_configs.csv') \
    else pd.DataFrame(columns=['Config', 'Status'])

remaining_configs = [c for c in df['Config'].values.tolist() if c not in run_configs]

for script_command in tqdm(remaining_configs, desc='Running tasks'):
    if script_command in df['Config'].values:
        print(f"Already run: {script_command}")
        continue
    ...
    if result.returncode == 0:
        print(f"Error running command: {script_command}")
    else:
        df.loc[df['Config'] == script_command, 'Status'] = 'Completed'
