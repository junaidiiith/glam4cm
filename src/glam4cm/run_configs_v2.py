...
TEXT_EXTRACTION_FLAG_PRESETS = [
    {},
    {"use_attributes": True},
    {"use_attributes": True, "use_node_types": True},
    {"use_attributes": True, "use_node_types": True, "use_edge_label": True},
    {
        "use_attributes": True,
        "use_node_types": True,
        "use_edge_label": True,
...
def model_dir_for_tokens(tokens: List[str], models_dir: str) -> str:
    ...
    config_id = get_config_hash(parsed_args, TASK_TYPES[llm_task_id])
...
def config_flag_combinations(args, dataset: str) -> List[Dict[str, bool]]:
    ...
    combinations = []
    seen = set()
    for preset in TEXT_EXTRACTION_FLAG_PRESETS:
        config = {flag: False for flag in CONFIG_FLAGS}
        config.update(preset)
        config = canonicalize_flags_for_dataset(dataset, config)
...
def average_numeric_results(results_by_run: List[Dict[str, object]]) -> Dict[str, object]:
    averaged = {}
    if not results_by_run:
        return averaged
    keys = set().union(*(result.keys() for result in results_by_run))
    for key in sorted(keys):
        values = [result.get(key) for result in results_by_run]
...
def execute_configs(items: List[Dict[str, object]], args):
    ...
    for item in tqdm(items, desc="Running configurations"):
        tokens = set_arg(list(item["tokens"]), "limit", args.limit)
        task_id = get_task_id(tokens)
        ...
        run_records = []
        flattened_results_by_run = []
        final_results_by_run = []
        final_config_params = initial_config_params
        final_tokens = tokens
        execution_status = "success"
        error = None
        
        seeds = Random(args.seed).sample(range(1, 2**args.runs - 1), args.runs)

        for run_number, seed in zip(range(1, args.runs + 1), seeds):
            tokens = set_arg(list(item["tokens"]), "seed", seed)
            print(f"Configuration run {run_number}/{args.runs}")
            run_item = dict(item)
            run_item["tokens"] = tokens
            result = execute_item(run_item, args, env)
            final_tokens = result["final_tokens"]
            final_config_params = parsed_config_for_log(final_tokens)
            ...
            run_record = {
                ...
                "final_command": " ".join(final_tokens),
            ...
            final_results_by_run.append(final_results)
            flattened_results_by_run.append(flattened_results)

        finished_at = datetime.now(timezone.utc).isoformat()
        averaged_results = average_numeric_results(flattened_results_by_run) \
            if execution_status == "success" and not args.dry_run else {}
        record = {
            "task_id": task_id,
            ...
            "runs": args.runs,
            "successful_runs": len(flattened_results_by_run),
            "attempts": [run_record["attempts"] for run_record in run_records],
            "run_records": run_records,
            "final_results": final_results_by_run,
        }
        record.update(flatten_config_params(final_config_params))
        record.update(averaged_results)
